...
type RepositoryConfigWriter interface {
	...
	InsertConfig(ctx context.Context, cfg *domain.RepositoryConfig) (*domain.RepositoryConfig, error)
	...
	// 0 rows (строки нет) → ErrNotFound.
	UpdateConfig(ctx context.Context, spec RepositoryConfigUpdate) (*domain.RepositoryConfig, error)
	...
	// исходной строки нет → ErrNotFound.
	RekeyConfig(ctx context.Context, registryID, oldName, newName string) (*domain.RepositoryConfig, error)
	// DeleteConfig снимает overlay-строку (DELETE ... RETURNING). 0 rows → ErrNotFound.
	DeleteConfig(ctx context.Context, registryID, name string) error
}
